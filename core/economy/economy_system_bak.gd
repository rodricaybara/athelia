...
func _calculate_flags(shop: ShopInstance, entity_id: String) -> Dictionary:
	return {
...
## Obtiene el valor base de un item
## TODO: Integrar con ItemRegistry cuando esté disponible
func _get_item_base_value(item_id: String) -> int:
	# Intentar obtener de ItemRegistry si existe
	var item_registry = get_node_or_null("/root/ItemRegistry")
	if item_registry and item_registry.has_method("get_item_definition"):
		var item_def = item_registry.get_item_definition(item_id)
		if item_def and item_def.has("base_value"):
			return item_def.base_value
	
	# Fallback: valores mock para testing
	var mock_values = {
		"health_potion": 50,
		"mana_potion": 60,
		"iron_sword": 200,
	...
	return mock_values.get(item_id, 10)


## Obtiene el nombre de un item
## TODO: Integrar con ItemRegistry + LocalizationSystem
func _get_item_name(item_id: String) -> String:
	# Intentar obtener de ItemRegistry si existe
	var item_registry = get_node_or_null("/root/ItemRegistry")
	if item_registry and item_registry.has_method("get_item_definition"):
		var item_def = item_registry.get_item_definition(item_id)
		if item_def and item_def.has("name_key"):
