...
## Mapeo de teclas a skill IDs
@export var skill_hotkeys: Dictionary = {
	"skill_1": "skill.attack.light",
	"skill_2": "skill.attack.heavy",
	"skill_3": "skill.combat.dodge",
	# "skill_4": "skill.combat.defend",
	# "skill_5": "skill.combat.flee",
}
...
func _request_skill_use(skill_id: String) -> void:
	# Validación 1: Debe haber un target válido
	if current_target.is_empty():
		print("[PlayerCombatController] Cannot use skill: no target selected")
		return
	
	# Validación 2: Fase correcta (doble check)
	if game_loop.get_current_phase() != GameLoopSystem.TurnPhase.PLAYER_ACTION_SELECT:
	...
	# Emitir evento de acción solicitada
	var action_data = {
		"actor": player_id,
		"skill_id": skill_id,
	...
	# Deshabilitar input hasta que termine la acción
